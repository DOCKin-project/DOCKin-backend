#!/usr/bin/env bash
#
# 밤 15 (#83) — 인스턴스 쪽 오케스트레이터. 본판 → 창 A·REINDEX·창 B → DONE 표식.
#
# [무엇이 밤 14와 다른가]
...
#
# [실행]  setsid nohup ./scripts/e8-night15.sh > ~/night15.log 2>&1 < /dev/null &
#
set -uo pipefail
cd "$(dirname "$0")/.."
...
DONE_FILE=${DONE_FILE:-$HOME/NIGHT15_DONE}

say() { printf '%s  %s\n' "$(date +%H:%M:%S)" "$*"; }
rm -f "$DONE_FILE"

sudo shutdown -c >/dev/null 2>&1 || true
sudo shutdown -h "+$WATCHDOG_MIN" "night15 watchdog" >/dev/null 2>&1 && say "watchdog: ${WATCHDOG_MIN}분 뒤 정지"

say "본판 시작 (MEM_RAISE=$MEM_RAISE)"
MEM_RAISE="$MEM_RAISE" SHUTDOWN_WHEN_DONE=0 ./scripts/e8-longrun-cause.sh 2>&1 | grep -v 'is obsolete' > "$HOME/night15-main.log"
...
say "창 A·B 시작"
./scripts/e8-reopen.sh "$MAIN/run.log" > "$HOME/night15-reopen.log" 2>&1
say "창 A·B 끝"

# e8-reopen.sh가 +10분 정지를 걸어 둔다. 로컬 fetcher가 그 안에 받고 terminate한다. 못 받으면 정지 → EBS에 남는다.
echo "NIGHT15 DONE $(date -u +%FT%TZ) main=$MAIN" > "$DONE_FILE"
