...
- **결과**: 🟢 **DB — 그것도 PostgreSQL이 아니라 컨테이너 cgroup.** 밤 14(2026-09-17): 앱 재시작 미회복, DB 재시작 즉시 회복, 체크포인트 requested 0·autovacuum 무관. 밤 15(2026-09-19): 매 분 cgroup을 적으니 file 캐시가 62k 청크에서 상한(512 − 공유메모리 142 ≈ 330MB 순수 캐시)에 붙고, 177k부터 회수 횟수가 분당 250 → 1,500으로 뛰며 INSERT 3 → 26ms. **재시작 없이 `docker update --memory 1g`** 한 줄로 다음 표본부터 2.8ms. 책의 언어로: 『막힘없이』 Ch.1의 버퍼 캐시 이야기는 `shared_buffers`(128MB, 그대로 두고 회복)가 아니라 그 바깥의 **OS 페이지 캐시**였고, 컨테이너에서는 그것이 cgroup 상한에 든다. `DB-IMPROVEMENT-PLAN.md` 6절 순서 6-2, `AWS-MEASUREMENT-RESULTS.md` 밤 14·15. 부수: 밤 4가 두 번 실패한 것은 rig(판정 문자열·락 키)였고 밤 14에서 고쳤다.
...
| 6 | 2026-08-13 밤 2 | 3시간 연속 색인에서 처리량 **1.8배 열화**(60 → 108.7ms/원본). 밤 3이 "코퍼스 크기 때문"을 반증 | **DB 컨테이너 512M 상한이 페이지 캐시까지 센다** — 힙+HNSW의 뜨거운 페이지가 ≈330MB에 안 들어가는 177k 청크부터 인덱스 읽기가 EBS로 간다(밤 15, 2026-09-19). 체크포인트·autovacuum·JVM은 아니다 | 밤 14: 앱 재시작 ✗ · DB 재시작 ✓. 밤 15: 재시작 없이 `docker update --memory 1g`로 다음 표본부터 회복. 운영 상한은 사용자 결정(B2) | **원인 확정** (밤 15) | 막힘없이 Ch.1·Ch.2 · Internals · Smith 체크포인트 장 |
