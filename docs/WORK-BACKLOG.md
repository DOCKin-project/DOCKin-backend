...
| P2-11-5 | **Testcontainers 도입** | DB 테스트가 개발자 로컬 PostgreSQL에 의존해 CI에서 통째로 skip된다. 도입하면 **P0-7이 "CI에 DB를 주지 않기로" 한 두 이유가 모두 해소된다** — 컨테이너 인자를 코드로 줄 수 있어 `lock_timeout` 문제가 사라지고, 태그로 벤치마크만 뺄 수 있다. P0-11 / P0-12를 한 번에 닫는다 | ☆ |
