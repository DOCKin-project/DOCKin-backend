...
코어 2배에 6~7배가 나왔다 — 코어 수만이 아니라 세대 차이(AVX-512, 메모리 대역)가 같이 들어간 값이다. **처리량은 동시 4(물리 코어 수)에서 포화**하고 8을 넣으면 지연만 두 배가 된다 — 모양은 i3와 같고 위치만 다르다. 이 값이면 4-2의 피크 10~30 req/s에 **2~6대**가 필요하다. 아직 부족하지만 자릿수는 맞아졌다.
...
**5배 빠르고 출력은 표본 4문장 전부 글자 하나까지 같다.** int8 양자화가 품질을 바꾸지 않았다는 뜻은 아니고(표본 4개다), 이 문장들에선 같았다는 것이다. 처리량은 **한 대에서 피크 가정 30 req/s에 닿는다.** 배치도 이 코어 수에선 산다 — 8문장 한 번에 건당 63ms(transformers 기준), 4-2의 "언어별 1회"를 방 단위 배치로 묶으면 더 내려간다.
