...
**어떻게 잴지는 정해 뒀다 (2026-09-17).** 실사용이 없어 지금은 못 잰다 — 파일럿(PRODUCTION-READINESS H1) 전엔 채팅 데이터가 시드뿐이다. 대신 그때 바로 돌릴 질의를 `measure/chat-translate-demand/demand.sql`에 두었다: 메시지마다 **방 멤버의 언어 집합 − 원문 언어**를 세어 분 단위(지속 피크, 서버 대수를 정한다)·초 단위(순간 피크, 큐 깊이를 정한다) 상위 20과 혼합 방 비율·혼합 방 메시지당 호출을 낸다. pylab ADR-0003 뒤로 서버는 어느 쌍이든 한 홉이라 피벗 곱하기는 없다. 멤버는 현재 `chat_members` 기준이라 입퇴장 이력이 없는 만큼 근사다.

그 전까지의 상한은 가정으로만 둘 수 있다: 피크 인바운드 30 msg/s(3-3) × 혼합 방 메시지 비율 × 혼합 방 메시지당 호출. 10명 방에 언어가 2~3개면 메시지당 호출은 발신자가 누구든 1~2다. 혼합 방 비율을 30%(외국인 비중)로 보면 **9~18 req/s**, 이 제품이 겨냥하는 현장처럼 방 대부분이 혼합이면 **30~60 req/s** — 그래서 T5~T7이 만든 여유(opus 한 대 93 req/s, NLLB 한 대 18 req/s)가 뜻이 있다. 위 질의가 이 범위의 어디인지를 정한다.
