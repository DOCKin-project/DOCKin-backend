...
**B2를 다시 쓴다** — `shared_buffers`는 그대로, 컨테이너 상한은 "힙 + HNSW + `shared_buffers` + 여유". 운영 `compose.yaml`은 아직 512M — 코퍼스가 ~17만 청크를 넘으면 운영 색인이 같은 자리에서 느려진다. 얼마로 올릴지(1G? 코퍼스 상한과 같이?)는 사용자 결정.
