...
--
-- ---------------------------------------------------------------------------
-- 주의 -- 이 파일을 고쳐도 이미 시드가 들어간 DB에는 반영되지 않는다.
--
-- 내용을 바꾸면 체크섬이 바뀌어 Flyway가 다시 실행하기는 한다. 그런데 모든 INSERT가
-- ON CONFLICT DO NOTHING이므로 **기존 행은 그대로 두고 아무 일도 하지 않는다.**
-- 로그에는 "Successfully applied 1 migration"이 찍히는데 데이터는 그대로다.
--
-- 이것은 결함이 아니라 의도다. DO UPDATE로 바꾸면 앱을 띄울 때마다 시드가
-- 사용자가 고친 데이터를 덮어쓴다. 보호받아야 하는 쪽은 앱이 만든 데이터다.
--
-- 그래서 시드 내용을 바꾼 뒤 반영하려면 해당 행을 지우고 다시 띄워야 한다.
-- document_chunks는 (source_type, source_id)로 다형 참조라 FK가 없고 CASCADE도 없으므로
-- 원본만 지우면 청크가 고아로 남는다. 함께 지운다:
--
--   DELETE FROM document_chunks
--    WHERE (source_type = 'WORK_LOG'             AND source_id IN (SELECT log_id FROM work_logs))
--       OR (source_type = 'WORK_LOG_TRANSLATION' AND source_id IN (SELECT translation_id FROM work_log_translations))
--       OR (source_type = 'SAFETY_COURSE'        AND source_id IN (SELECT course_id FROM safety_courses));
--   DELETE FROM work_log_translations;
--   DELETE FROM work_logs;
--   DELETE FROM safety_courses;
...
--   DELETE FROM users;
--
-- 그다음 앱을 다시 띄우면 Flyway가 처음부터 넣는다. 9절의 setval이 시퀀스도 맞춘다.
-- ---------------------------------------------------------------------------
--
...
--
-- 길이를 일부러 섞었다. 청킹은 (제목 + 개행 + 본문) 전체를 대상으로 하고 TARGET_CHARS가
-- 500이므로, 짧은 것은 1청크가 되고 log_id 14는 1,000자를 넘겨 3청크가 된다.
--
-- 3청크짜리를 하나 둔 이유는 RetrievalService의 MAX_PER_SOURCE(=2)가
-- **청크가 셋 이상이어야 비로소 실행되는** 코드이기 때문이다. 2청크까지는 상한에 걸리지 않아
-- 그 분기가 한 번도 돌지 않는다. log_id 3과 9(450~470자)는 1청크로 남겨 대비를 만든다.
--
-- (처음에는 이 주석이 "3/9/14가 900자를 넘겨 2~3청크"라고 적혀 있었으나 실측은 450~517자였다.
--  한국어 글자 수를 눈대중으로 센 탓이고, 색인을 실제로 돌려보고서야 드러났다.)
-- ---------------------------------------------------------------------------
INSERT INTO work_logs (log_id, title, log_text, audio_file_url, created_at, updated_at, user_id, equipment_id) VALUES
  ...
  (14, '야간 도장 작업 중 정전 대응 기록',
   '새벽 2시 15분경 도장공장 일부 구역에 정전이 발생했다. 도장설비와 환기 장치가 동시에 정지했고 비상등만 점등된 상태였다. 도장 부스 내부에 작업자 두 명이 있었으며 환기가 멈춘 상태에서 유기용제 증기가 체류할 위험이 있어 즉시 대피시켰다. 대피는 정전 발생 후 약 90초 이내에 완료되었다. 전기팀 확인 결과 도장공장 배전반의 누전차단기가 동작한 것이었고, 원인은 옥외에 방치된 임시 배선의 절연 파괴였다. 강우로 물이 유입되면서 절연이 깨진 것으로 보인다. 임시 배선을 철거하고 차단기를 복구한 시각은 새벽 3시 40분이며, 복구 후 환기 장치를 30분 가동해 부스 내부 농도를 측정하고 정상을 확인한 뒤 작업을 재개했다. 이번 건에서 문제로 남는 것은 정전 시 환기 장치가 비상 전원 계통에 물려 있지 않다는 점이다. 대피가 빨라 사고로 이어지지 않았을 뿐, 부스 안쪽 깊은 곳에서 작업 중이었다면 상황이 달랐을 것이다. 환기 장치를 비상 전원에 포함시켜 달라고 정식으로 요청했다. 대피 과정에서 확인된 문제도 함께 적어 둔다. 비상등이 점등되기까지 약 4초의 공백이 있었고 그동안 부스 내부는 완전한 암흑이었다. 작업자들이 손으로 벽을 더듬어 출구를 찾았다고 진술했다. 축광 유도 표지가 부스 내부에는 부착되어 있지 않아 방향을 알 수 있는 수단이 없었다. 통로 쪽에는 있으나 부스 안쪽에는 없다. 야간 도장은 부스 안에서 하는 작업이므로 표지가 필요한 곳도 부스 안이다. 안전관리팀에 축광 표지 추가 부착을 요청했고 견적을 받기로 했다. 그리고 이번 정전으로 도장 중이던 블록 두 개의 도막 상태를 확인해야 한다. 도장 도중 건조 조건이 끊기면 도막에 결함이 생길 수 있다. 정전 시각이 도장 직후 30분 이내였는지 확인이 필요하며, 그 경우 해당 구간은 재도장 대상이 된다. 작업 기록을 대조한 결과 한 블록은 도장 완료 후 2시간이 지난 상태였고 다른 하나는 도장 중이었다. 도장 중이던 블록은 다음 조가 도막 두께와 외관을 검사한 뒤 판정하기로 했다.',
