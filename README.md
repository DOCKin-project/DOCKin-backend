...
**해결**: 문제가 된 두 필드(마지막 메시지, 마지막 읽음 시간) 갱신을 `@Modifying` 네이티브 UPDATE 쿼리로 바꿔서, JPA 영속성 컨텍스트와 dirty checking을 아예 거치지 않게 했습니다. 당시 남긴 주석이 의도를 그대로 보여줍니다:

> "여기서 핵심은 room 객체의 필드를 절대 setter로 고치지 않는 것입니다!"

낙관적 락이나 명시적 락을 추가한 게 아니라, 애초에 JPA가 관리 엔티티를 갱신하면서 락을 걸 상황 자체를 만들지 않는 쪽으로 우회한 방식입니다.

같은 시기에 확인한 두 번째 실제 버그: 댓글이 달린 작업일지가 삭제되지 않는 문제. `Work_logs`에 `comments` 연관관계(`@OneToMany(cascade=ALL, orphanRemoval=true)`)가 빠져 있어 Hibernate가 자식 댓글의 존재를 모른 채 부모 삭제를 시도했던 게 원인이었고, 연관관계를 추가하고 FK를 `ON DELETE CASCADE`로 바꿔 해결했습니다.

> 아직 정리 안 된 부분: `saveMessage`는 지금도 `@Async`로 선언돼 있는데, 로그 문구는 "동기 방식으로 실행"이라고 남아 있습니다. 락 픽스 과정에서 실행 방식이 바뀌었는데 주석/로그가 못 따라간 흔적으로 보이고, 의도된 설계는 아닙니다.
