...
| P2-15-2 | **`equipment_id`가 NULL이면 목록 페이지 전체가 500** | **측정 중 발견.** 아래 별도 | ★ |
| P2-15-3 | **`GET /api/work-logs`에 정렬이 없다** | **측정 중 발견.** `@PageableDefault(direction=DESC)`에 `sort`가 비어 있다 | ★ |
...
### P2-15-2 — 장비 없는 일지 하나가 페이지 전체를 500으로 만든다

`WorkLogDto.from`(`WorkLogDto.java:51`)이 `entity.getEquipment().getEquipmentId()`를
**무조건** 부른다. `equipment_id`는 `V2__baseline_existing_tables.sql`에서 **nullable**이고,
...
> **스키마가 허용하는 상태를 코드가 가정으로 배제하고 있다.** 이 저장소가 반복해서 잡아 온
...
`sort` 속성이 비어 있다. **방향만으로는 정렬이 만들어지지 않으므로** 이 Pageable은
unsorted다. 즉 `GET /api/work-logs`와 `/others/{id}`는 **정해지지 않은 순서로** 응답한다
(`/search`만 `sort = "createdAt"`이 있다).
...
**다음 실행 전에 고칠 것** — `cleanUp`이 삭제 전에 FK 인덱스를 만들어야 한다(P2-15-4).
지금 형태로는 측정보다 뒷정리가 오래 걸린다.
