...
| P2-8-2 | **원문과 번역본이 top-k 자리를 중복으로 차지한다** | `selectTopK`의 키가 `sourceType + ":" + sourceId`다. 원문은 `WORK_LOG:7`, 번역본은 `WORK_LOG_TRANSLATION:1`이라 **서로 다른 문서로 취급**된다. 같은 내용이 언어만 바꿔 두 자리를 먹는데 top-k가 5뿐이다. 청크만 봐서는 번역본의 원본 `log_id`를 알 수도 없다 |
