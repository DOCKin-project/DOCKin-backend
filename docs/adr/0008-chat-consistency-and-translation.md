...
같은 자리에서 ③의 후보를 하나 재봤다. 피벗 오역("용접→비행기", "조립→회의")을 절반 넘게 없애는 NLLB-600M(pylab T3, ko→vi 뜻 보존 31 vs 16)은 **건당 291ms, 한 대 4.2 req/s** — opus 피벗 ko→vi(144ms, 12.5~17 req/s)의 1/3~1/4. 4-2의 피크 30 req/s를 NLLB로 받으려면 7대다. 그래서 4-2의 "30 req/s"가 가정인 채로는 모델을 못 고른다 — **자동 번역이 실제로 초당 몇 건인지**(비영어 수신자가 있는 메시지 비율 × 피크)가 D3의 다음 숫자다. 모델 결정(opus+사전 / NLLB / 쌍별 섞기)은 pylab ADR-0001 7·8절에서, 이 ADR은 그 결과를 받는다. 같은 날 T4(밤 7)가 배치로 문장당 2.9배를 보였고, T5(밤 8)가 그것을 서버 안 마이크로배칭(창 20ms)으로 실제로 먹었다 — NLLB 한 대 4.2→9.2(동시 8)·18.3 req/s(동시 32), 한가할 때 +14ms. 7대가 **2~3대**가 됐다. 이제 모델 결정의 마지막 변수는 4-2의 피크가 실제로 몇 req/s인가와, 자동 번역이 기다려도 되는 지연이 0.8초인가 1.5초인가다. T6(밤 9)는 같은 배치가 HTTP 경로에서도 먹는 것을 확인했다 — opus ko→en **한 대 60 req/s**(31에서), 피벗 30~32. 팀원 서버에 보낼 요청은 이제 넷이다: CTranslate2 int8, 마이크로배칭(창 20ms), 직접 모델, 그리고 `asyncio.to_thread` 풀 크기(기본 cpu+4=12가 동시 요청의 상한이 된다 — T7·밤 10, 이걸 64로 두자 한 대 ko→en 93~96 req/s·피벗 50). 밤 10이 남긴 동시 128 붕괴는 T8(밤 11)에서 서버 쪽이 아니라 **호출하는 쪽이 유휴 keep-alive 연결을 오래 들고 있다가 다시 쓰는 것**이 조건이라고 나왔다(keepalive 256·동시 192에서 4/4 붕괴 66 req/s·p95 6초, 20이면 4/4 멀쩡) — 피크 30엔 닿지 않는다. 이어 T10(밤 12)이 그 6초를 요청별 구간으로 갈라 보니 **서버가 아니라 벤치 클라이언트 httpx의 커넥션 풀 대기**였다(느린 요청 144/144, 서버 구간·루프 지연은 정상) — 밤 11의 "keep-alive 풀은 작게" 권고는 거둔다. 운영에서 느리면 서버 p95보다 부르는 쪽의 풀 대기를 먼저 본다.
