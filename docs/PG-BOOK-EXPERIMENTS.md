...
- **결과**: 🟢 **DB — 그것도 PostgreSQL이 아니라 컨테이너 cgroup.** 밤 14(2026-09-17): 앱 재시작 미회복, DB 재시작 즉시 회복, 체크포인트 requested 0·autovacuum 무관. 밤 16(2026-09-19): 매 분 cgroup을 적으니 file 캐시가 62k 청크에서 상한(512 − 공유메모리 142 ≈ 330MB 순수 캐시)에 붙고, 177k부터 회수 횟수가 분당 250 → 1,500으로 뛰며 INSERT 3 → 26ms. **재시작 없이 `docker update --memory 1g`** 한 줄로 다음 표본부터 2.8ms. 책의 언어로: 『막힘없이』 Ch.1의 버퍼 캐시 이야기는 `shared_buffers`(128MB, 그대로 두고 회복)가 아니라 그 바깥의 **OS 페이지 캐시**였고, 컨테이너에서는 그것이 cgroup 상한에 든다. `DB-IMPROVEMENT-PLAN.md` 6절 순서 6-2, `AWS-MEASUREMENT-RESULTS.md` 밤 14·15. 부수: 밤 4가 두 번 실패한 것은 rig(판정 문자열·락 키)였고 밤 14에서 고쳤다.
...
- **결과 ✅ (2026-09-15, 로컬 279MB·4판)**: `cost_delay` 2ms → 0으로 VACUUM 자체가 **24.6/25.7s → 8.2/13.2s (2~3배)**. 네 판의 일이 동일해 차이는 전부 `cost_delay`고, buffer usage로 센 "일부러 자는 시간" 10.8s가 실측 차이와 맞는다. 힙은 네 판 모두 그대로(앞쪽 구멍). 깨어남은 34~52s(launcher 위상).
  ...
  - **A2 결정(2026-09-19, 추천대로)**: 기본값 유지 — 15초를 아끼려 I/O를 3배 쓰는 것. `DB-IMPROVEMENT-PLAN.md` 4절.
...
- **결과** (2026-09-16, `pg_trgm`): 🟢 100만 행 A B A B, `measure/trgm/trgm-20260916T135051/README.md`.
  - GIN이 계획을 바꾼 건 **희귀 4자**뿐 — 실제 검색 SQL에서 241/300ms → 1.7/1.7ms(160배). 희귀 3자는 안 쟀다 `[실측 필요]`. 흔한 3자는 통계 표본에 따라 GIN을 쓰기도 안 쓰기도(122↔261ms). **2자는 트라이그램이 없어 무효** — 한국어 검색어의 흔한 길이다.
  - 대가: `title` 24MB + `log_text` 157MB(힙의 39%), 벌크 쓰기 4.5배, CONCURRENTLY 빌드 180s(일반의 2배), 512MB에서 캐시 경합 가능성`[추정]`.
  - **책의 언어로**: `LIKE '%kw%'`가 B-tree를 못 타는 건 맞지만, 이 쿼리의 비용은 그 자리가 아니었다. Bitmap Heap Scan이 구역 84명의 행이 흩어진 3만 페이지를 읽고 `ORDER BY`가 `LIMIT`의 조기 종료를 막는다 — 실행계획을 읽기 전엔 "키워드 인덱스"가 답으로 보였다. **D1 결정(2026-09-19, 추천대로): 지금은 안 붙인다.** 느린 자리 자체는 #91.
  - **후속 결과** (2026-09-20, #91·#118, rig `scripts/db/area-denorm-lab.sh`, `measure/area/area-20260919T234301/README.md`): 구역 필터 자리 셋(IN 목록 / #127 조인+V10 / 구역 컬럼 비정규화)을 100만 행·구역 17%·50%·1%에서. 17%·50% 구역의 목록과 흔한 검색은 #127로 끝(17% 목록 358→1.7ms, 비정규화해도 같다). 비정규화가 이기는 건 1% 소구역(목록 7→0.7ms, 흔한 검색 76→1.7ms)과 17% 구역의 희귀 검색(1.3~2.6s→0.3~0.5s)뿐이고, 50%·1% 구역의 "희귀" 검색은 결과 0건인 검색이라 셋 다 같이 느리다 → 안 한다 추천. **책의 언어로, 다시**: 정렬 키가 앞인 인덱스 + `LIMIT`는 "조기 종료"의 도박이다 — 조건에 맞는 행이 드물수록(없으면 끝까지) 인덱스 순서로 힙 전체를 두드린다. 100만 행·1g에서 2.1s, 512m에선 31~47s(#83과 같은 메커니즘). 『SQL TUNER』의 "LIMIT 조기 종료"는 선택도가 높다는 전제를 같이 읽어야 한다.
...
| 6 | 2026-08-13 밤 2 | 3시간 연속 색인에서 처리량 **1.8배 열화**(60 → 108.7ms/원본). 밤 3이 "코퍼스 크기 때문"을 반증 | **DB 컨테이너 512M 상한이 페이지 캐시까지 센다** — 힙+HNSW의 뜨거운 페이지가 ≈330MB에 안 들어가는 177k 청크부터 인덱스 읽기가 EBS로 간다(밤 16, 2026-09-19). 체크포인트·autovacuum·JVM은 아니다 | 밤 14: 앱 재시작 ✗ · DB 재시작 ✓. 밤 16: 재시작 없이 `docker update --memory 1g`로 다음 표본부터 회복. 운영 상한은 사용자 결정(B2) | **원인 확정** (밤 16) | 막힘없이 Ch.1·Ch.2 · Internals · Smith 체크포인트 장 |
