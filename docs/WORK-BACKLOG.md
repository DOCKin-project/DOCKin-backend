...
| P2-11-2 | **관측 수단이 전무** | `actuator`·`micrometer` 의존성이 없다. **헬스 체크 엔드포인트도, 풀/GC/힙 메트릭도 없다.** P2-9-1이 실제로 터졌을 때 원인을 찾을 수단이 없다 | ★ |
| P2-11-3 | **`traceId`를 절반만 쓰고 있다** | DTO 필드로 존재해 FastAPI로 넘어가고 `chat_history`에도 남지만, **MDC를 안 써서 애플리케이션 로그에는 없다.** 로그로 요청을 이을 수 없다. 이미 있는 값을 잇기만 하면 된다 | ★ |
...
| 함께 | 기동 시 빌드 정보를 로그에 남긴다(Actuator `info` 또는 `build-info`). **지금은 도는 코드의 버전을 알 방법이 없다** — P2-11-2와 같은 자리 |
