...
건당 수백 ms라고 가정해도(**실측은 그보다 나쁘다 — 7-3절, 건당 2초**) 30 msg/s 피크에서 번역을 저장 경로에 동기로 붙이면
...
| M7 | FastAPI 번역 **건당 지연과 동시 처리량** — D3를 붙이기 전에 그 서버가 무엇인지 | 4-2의 10~30 req/s를 받을 수 있는 서버인가 | **완료 (2026-09-16)** — 7-3절. 이 머신에서 **p50 1.9초, 처리량 0.7 req/s, 동시성을 올려도 늘지 않는다.** 가정치의 1/15~1/40. D3는 이 상태로는 못 붙인다 |
...
### 7-3. M7 결과 — FastAPI 번역은 건당 2초, 초당 0.7건, 동시성으로 안 늘어난다 (2026-09-16)
