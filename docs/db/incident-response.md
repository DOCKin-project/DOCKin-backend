...
| 보이는 것 | 원인 | 이 저장소에서 있었던 일 | 조치 |
|---|---|---|---|
| ⑤에서 미스/건은 같고 ms/건만 10배+, `memory.stat`의 file이 상한 근처에서 고정, `memory.events`의 max가 분당 수백~수천씩 는다 | **DB 컨테이너 메모리 상한이 페이지 캐시까지 세서 인덱스 읽기가 디스크로 간다** (밤 15에서 확정) | 밤 2·14·15: 17만 청크부터 INSERT/건 3 → 26ms, 앱 재시작 미회복, DB 재시작 즉시 회복(장애 #6). 밤 15: 재시작 없이 상한만 1G로 올려 다음 표본부터 회복 | **재시작 대신 `docker update --memory <힙+HNSW+shared_buffers+여유> --memory-swap <같은 값> dockin-db`** — 무중단, 다음 표본부터 돌아온다. `compose.yaml`의 값도 같이 올려 두지 않으면 다음 `up`에서 되돌아간다(B2) |
...
**해 본 기록.** 밤 14(2026-09-17, `AWS-MEASUREMENT-RESULTS.md`): 앱 재시작 → 12분 미회복 → DB 재시작 → 즉시 회복. 밤 15(2026-09-19): 위 네 번째 명령(cgroup)을 매 분 찍으며 재현, `docker update --memory 1g`만으로 회복 — 첫 행은 이제 실측이다.
