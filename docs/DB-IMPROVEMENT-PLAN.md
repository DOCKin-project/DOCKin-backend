...
| **B2** | **`shared_buffers`는 안 올린다.** 컨테이너 512M도 그대로 | 밤 3: 128MB → 1GB, 컨테이너 4배에도 처리량 불변 | — | 결정 (4절) — **단, "컨테이너 512M 그대로"는 다시 본다.** 밤 14: 512M 안에서 힙+HNSW가 페이지 캐시를 넘기는 자리(≈177k 청크)부터 INSERT가 15배 느려졌다 `[추측, #83]`. 밤 3의 "불변"은 조건마다 DB를 새로 띄운 탓에 검증이 아니었다. `shared_buffers`는 여전히 안 올린다 — 미스 수는 앞뒤가 같았다 |
...
| `pg_trgm` GIN을 `work_logs`에 지금 | 계획이 바뀐 건 **희귀 4자** 키워드뿐(160배, 희귀 3자는 `[실측 필요]`). 흔한 3자는 통계 표본 따라 120↔260ms 변동, 2자는 트라이그램이 없어 무효. 대가는 힙의 39%·벌크 쓰기 4.5배·512MB 캐시 경합 가능성`[추정]`. 실제 검색의 240ms는 LIKE가 아니라 구역 필터+정렬이다. **추천만, 결정은 사용자** | D1 |
...
**D1 판단(추천, 결정은 사용자)**: 지금은 안 붙인다 → 4절. 잰 것 중 계획이 바뀐 건 희귀 4자뿐이고(희귀 3자 `[실측 필요]`), 한국어 검색어는 2자가 흔하다(설비·부위·증상).
...
**남은 것.** 카드 ③의 관측 스레드(기존 두 테스트에 붙여 산출물 파일로), #83 cgroup 재실험(+창 A·B), #89. 순서표는 다 지웠다 — 다음은 **A2·D1 결정**(둘 다 추천은 "안 한다")과 E1(H1 트리거)뿐이다.
