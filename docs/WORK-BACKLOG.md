...
### P2-6 — 근무일 캘린더 (신규, 운영 투입 전 필수)

**현재 결근 배치는 주말만 제외하고 공휴일을 판단하지 못한다.** 이대로 운영에 넣으면
**공휴일에 전원이 결근 처리된다.** 이 프로젝트에는 근무일 정보가 어디에도 없다 —
`WorkShift`는 교대 시간대만 정의하고 휴무일·공휴일 개념이 없다.
ADR-0005가 지적한 "근무 정책 엔진 미충족"과 같은 뿌리의 문제이며, P3의 정책 엔진과 함께 다뤄야 한다.
